...
export default function ExpenseTable() {
  ...
  const { updateItem, userData, ViewItem } = Data;
  ...
  const DeleteItem = async (data) => {
    const apiResult = await dispatch(deleteExpense(data));
    if (deleteExpense.fulfilled.match(apiResult)) {
  ...
  const FilterByTimeline = async (data) => {
    ...
    const apiResult = await dispatch(FilterExpense(newData));
    if (FilterExpense.fulfilled.match(apiResult)) {
    } else {
      const errorMessage = apiResult?.payload || 'Something went wrong';
  ...
  const checkIfUserIsAuthorized = async () => {
    try {
      const response = await axiosInstance.get(
        `/google/auth/google-auth-status/${userData._id}`
      );
  ...
  const EventScheduler = async (data) => {
    const userIsAuthorized = await checkIfUserIsAuthorized(); 

    if (!userIsAuthorized) {
      window.location.href = `http://localhost:5000/api/google/auth/google`;
    } else {
      try {
        const response = await axiosInstance.post(`/expense/event`, data);
